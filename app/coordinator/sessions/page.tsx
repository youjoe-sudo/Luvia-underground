...
export default async function CoordinatorSessionsPage() {
  ...
  const { data: sessions } = cohortIds.length
    ? await supabase
        ...
        .select(
          'id, title, start_time, is_link_active, status, is_attendance_confirmed, cohorts(group_name), courses(title)'
        )
              ...
              <li key={s.id} className="flex items-center justify-between py-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge tone={s.is_attendance_confirmed ? 'info' : 'warning'}>
                    {s.is_attendance_confirmed ? 'Confirmed' : 'Pending'}
                  </Badge>
                </div>
              </li>
            ))}
